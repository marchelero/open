...
description: 'Use when auditing GitHub Actions workflow efficiency — recommends fixes to reduce CI minutes and costs.'
triggers: [ci, ci-cd, github actions, workflow yaml, runner minutes, caching, matrix, concurrency, pipeline, build time, gh run, actions]
...
## Security Guardrails (pack addition)

Efficiency never overrides pipeline security:

1. `permissions:` stays default-deny; escalate per job only.
2. Third-party actions pinned to a commit SHA; `@latest`/bare tags are a supply-chain hole — merging a matrix leg does not excuse upgrading one.
3. Cloud deploys via OIDC role assumption, not stored static keys.
4. Secrets referenced via `env:`, never interpolated into `run:` text or Docker `args:`.
5. Deploy jobs stay split plan (PR) → apply (main, environment-gated); never "optimize" the approval gate away.
