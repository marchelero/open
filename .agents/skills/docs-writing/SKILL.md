...
- **IS NOT:** editorial blog tutorials or articles (out of pack scope), a README from scratch or a whole-README rewrite (delegate to the doc-updater agent; a README that needs its prose fixed in place stays here), AGENTS.md or CLAUDE.md instructions (instruction files are execution-first, not reader-facing docs — see the pack-reference skill), marketing and landing-page copy (the marketing-agent covers it), or the product's own error strings and CLI output (developer-experience audits are out of scope; this skill covers the docs that describe them).
...
- A README from scratch or a full rewrite: the doc-updater agent; this skill audits and fixes existing ones in place.
- AGENTS.md/CLAUDE.md instruction files: execution-first, not reader-facing docs (see the pack-reference skill).
